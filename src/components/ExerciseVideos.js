...
const ExerciseVideos = () => {
  ...
  return (
    <Box sx={{ marginTop: { lg: '50px', xs: '20px' } }} p="36px">
      ...
      <Stack sx={{ flexDirection: { lg: 'row' }, gap: { lg: '25px', xs: '0px' } }} justifyContent="flex-start" flexWrap="wrap" alignItems="center">
        {exerciseVideos?.slice(0, 3)?.map((item, index) => (
          <a
            ...
            rel="noreferrer"
            >
            <img style={{ borderRadius: '8px' }} src={item.video.thumbnails[0].url} alt={item.video.title} />
            <Box>
